...
void Nrlmsise::spline(double *x, double *y, int n, double yp1, double ypn, double *y2) {
	...
	for (int i = 1; i < (n - 1); i++) {
		...
		y2[i] = (sig - 1.0) / p;
		u[i] =
		  (6.0 * ((y[i + 1] - y[i]) / (x[i + 1] - x[i]) - (y[i] - y[i - 1]) / (x[i] - x[i - 1])) / (x[i + 1] - x[i - 1]) - sig * u[i - 1]) /
		  p;
	}
...
double Nrlmsise::densm(double alt, double d0, double xm, double &tz, int mn3, double *zn3, double *tn3, double *tgn3, int mn2, double *zn2,
					   double *tn2, double *tgn2) {
...
void Nrlmsise::gtd7(NrlmsiseInput &input, NrlmsiseConfig &flags, NrlmsiseOutput &output) {
	...
	meso_tgn2[1] = pavgm[8] * pma[9][0] * (1.0 + flags.sw[20] * flags.sw[22] * glob7s(pma[9], input, flags)) * meso_tn2[3] * meso_tn2[3] /
...
void Nrlmsise::gts7(NrlmsiseInput &input, NrlmsiseConfig &flags, NrlmsiseOutput &output) {
	...
	if (input.alt < 300.0) {
		...
		meso_tgn1[1] = ptm[8] * pma[8][0] * (1.0 + flags.sw[18] * flags.sw[20] * glob7s(pma[8], input, flags)) * meso_tn1[4] * meso_tn1[4] /
